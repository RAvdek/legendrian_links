...
LOG = utils.LOG
GROEBNER_TIMEOUT_DEFAULT = 10


@utils.log_start_stop
def zero_set(polys, symbols, modulus=2, existence_only=False, groebner_timeout=GROEBNER_TIMEOUT_DEFAULT):
    """
    ...
    if len(symbols) == 0:
        return []
    root = SolutionSearchNode(polys, symbols, modulus=modulus, subs_dict=dict(), groebner_timeout=groebner_timeout)
    nodes = [root]
    spawned_nodes = []
    solution_nodes = []
    loop_counter = 0
...
class SolutionSearchNode(object):
    ...
    def _apply_subs(self):
        """Update self.polys by applying substitutions in self.subs_dict
