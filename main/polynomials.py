...
@utils.log_start_stop
def chunky_zero_set(polys, symbols, modulus=2, groebner_timeout=GROEBNER_TIMEOUT_DEFAULT, chunk_size=5):
    """
    Enumerate points in affine space generated by `symbols` in the intersection of the varieties
    determined by the polys. Like most of our code, this is a knock-off of Sivek's lch.sage code.
    ...
    The chunky implementation proceeds by iteratively adding new variables and new polynomials.
    ...
    # We can rank this more intelligently
    analyzed_polys = []
    ...
    while len(analyzed_polys) < len(polys) and len(subs_dicts) > 0:
        ...
        while len(new_polys) == 0:
            ...
            if count != 0:
                # what symbols appear in the polynomials with minumum amounts of new vars
                new_symbols = set()
                ...
                new_symbols = sorted(list(new_symbols), key=lambda sym: new_symbols_counts[sym], reverse=True)
                # Eliminate already analyzed symbols... Since count!=0 should be non-empty
                new_symbols = [s for s in new_symbols if s not in analyzed_symbols]
        ...
        LOG.info(f"Analysis on {loop_counter}th iteration of chunky_zero_set:\n"
...
@utils.log_start_stop
def filtered_zero_set(
        polys, symbols, modulus=2, existence_only=False,
        groebner_timeout=GROEBNER_TIMEOUT_DEFAULT):
    """
    Successively find zero sets by adding more polynomials and symbols

    :param polys: List of list of sympy polynomials. Must have commutative variables.
    :param symbols: List list of sympy symbols.
    :param modulus: Coefficient modulus.
    :param existence_only: Boolean. Are we just checking existence?
    :param groebner_timeout: compute Groebner bases until timeout is reached. If failed try larger number of simpler problems.
    :return: List of points in the intersection of varieties if existence_only==False else Boolean
    ...
    f_level = 0
    solution_nodes = []
    while f_level < len(polys):
        f_polys += polys[f_level]
        f_symbols += symbols[f_level]
        LOG.info(f"Searching for zero set of {len(f_polys)} polynomials in {len(f_symbols)} variables")
        nodes = [
            SolutionSearchNode(
                polys=f_polys,
                symbols=f_symbols,
        ...
        loop_counter = 0
        while True:
            LOG.info(f"{f_level}th level of augmentation search: "
                     f"{loop_counter}th loop of zero_set search: {len(nodes)} nodes, "
                     f"{len(spawned_nodes)} spawned nodes, "
                     f"{len(solution_nodes)} solution nodes")
            loop_counter += 1
            unspawned_nodes = [node for node in nodes if node not in spawned_nodes]
            ...
            for node in unspawned_nodes:
                if node.has_solution():
                    solution_nodes.append(node)
                    # Terminate as early as possible if only checking existence
                    if existence_only and f_level == len(polys):
                        return True
                nodes += node.get_spawn()
                spawned_nodes.append(node)
        subs_dicts = [node.subs_dict for node in solution_nodes]
        f_level += 1
    if existence_only:
        return len(solution_nodes) > 0
    return [node.subs_dict for node in solution_nodes]
