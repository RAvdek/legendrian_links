...
def zero_set(
        ...
        allow_unset=False, log_frequency=1000):
    """
    ...
    n_analyzed_nodes = 0
    while True:
        if loop_counter % log_frequency == 0:
...
class SolutionSearchNode(object):
    ...
    def _simplify_polys(self):
        ...
        ideal_basis_thread.daemon = True
        ideal_basis_thread.start()
        ideal_basis_thread.join(self.groebner_timeout)
        if ideal_basis_thread.is_alive():
            thread_timeout = True
    ...
    def _check_for_linear_polys(self):
        ...
        modified = False
        for c in self.ff_elements:
            # TODO: it's faster to check (p - c).is_symbol, maybe? Typically there will be more vars than polys
            # may also be faster to combine const_poly and this method
            for g in self.get_unset_vars():
